...
for i=1:Dk-1;
    for j=1:Dp;
        ...
        if dgrid(j) < g1(1);
        F1(j,i+1) = 0; %QQ(1,1)*0 +QQ(2,1)*LIP(dgrid,F2(:,i),LIP(g2,mgrid,dgrid(j)));
        F2(j,i+1) = 0; %QQ(1,2)*0+QQ(2,2)*LIP(dgrid,F2(:,i),LIP(g2,mgrid,dgrid(j))); 

        elseif dgrid(j) >= g1(1) & dgrid(j)<g2(Np);
        m1 = LIP(dgrid,F1(:,i),LIP(g1,mgrid,dgrid(j)));
        m2 = LIP(dgrid,F2(:,i),LIP(g2,mgrid,dgrid(j)));
        %here is the updating change
